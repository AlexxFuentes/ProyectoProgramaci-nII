...
public class GestionCuentaNomina {
	...
	public CuentaNomina BuscarCuentaNomina(NumeroCuenta numCuenta) {
		...
		for(int i = 0; i < listaCuentaNomina.size(); i++) {
			if( listaCuentaNomina.get(i).get_usuario().get_crearCuenta().get_numerocuenta().equals(numCuenta)) {
	...
	public boolean AgregarCuentaNomina(CuentaNomina cuentaNomina) {
		if(BuscarCuentaNomina(cuentaNomina.get_usuario().get_crearCuenta().get_numerocuenta()) == null) {//Si el usuario no esta agregado
	...
	public boolean ModificarCuentaAhorro(CuentaNomina CuentaNominaModificado, int posicion) {
		...
		if(posicion<0 || posicion>listaCuentaNomina.size()) {
			...
			if(BuscarCuentaNomina(CuentaNominaModificado.get_usuario().get_crearCuenta().get_numerocuenta()) != null) {
